                      ...
                      <div className="hidden sm:block space-y-1">
                        {visibleItems.map((item) => {
                          if (item.type === "timetable") {
                            const theme = getColorStyle(item.color)
                            return (
                              <div
                                key={item.id}
                                className={`text-label md:text-micro font-bold px-2 py-1 rounded border truncate flex items-center gap-1 ${theme.badgeBg}`}
                              >
